...
async function scrapeGoogleMaps(customSearches = null) {
  ...
  // Filtrăm doar lead-urile cu telefon mobil românesc
  const mobileLeads = items.filter(p => {
    const phone = (p.phone || p.internationalPhoneNumber || '').replace(/\s/g, '');
    return phone.startsWith('+407') || phone.startsWith('07') || phone.startsWith('00407');
  });
  
  console.log(`📱 Filtrare mobil: ${mobileLeads.length}/${items.length} au număr mobil`);
  
  return mobileLeads.map(p => ({
